...
export function SentenceBreakdownCard({
    ...
    return (
      <div className="relative flex h-full flex-col items-center justify-center rounded-2xl border-2 border-dashed border-[#d9d9d9] bg-[#fafafa] p-6 text-center">
        ...
        <p className="mt-1 max-w-xs text-xs font-medium leading-relaxed text-ash">
          Click vào bất kỳ câu nào để AI phân tích ngữ pháp, dịch nghĩa và bóc tách từ vựng.
        </p>
  ...
  // 5. Giao diện hợp nhất tất cả trong một Panel duy nhất (Unified Inspector)
  return (
    <div className="flex h-full flex-col rounded-2xl border-2 border-b-4 border-[#e5e5e5] bg-white shadow-xs overflow-hidden">
      ...
      {/* Vùng nội dung cuộn duy nhất hiển thị cả 3 phần: Ngữ nghĩa + Ngữ pháp + Từ vựng */}
      <div className="flex-1 overflow-y-auto p-3.5 space-y-4">
        {/* PHẦN 1: BẢN DỊCH NGỮ NGHĨA */}
        <section className="space-y-2">
          <div className="flex items-center justify-between">
            <span className="flex items-center gap-1.5 text-[11px] font-black uppercase tracking-wider text-[#438f0e]">
              <Languages className="size-3.5" /> 1. Bản dịch ngữ cảnh
            </span>
          </div>
        ...
        <section className="space-y-2.5 pt-2 border-t border-[#eeeeee]">
          <div className="flex items-center justify-between">
            <span className="flex items-center gap-1.5 text-[11px] font-black uppercase tracking-wider text-[#087db4]">
              <Layers className="size-3.5" /> 2. Cấu trúc ngữ pháp
            </span>
          </div>

          {(!data.grammar?.clauses || data.grammar.clauses.length === 0) && isEnriching ? (
            <div className="rounded-xl border border-[#bfe9fd] bg-[#f0f9ff] p-4 text-center space-y-1.5">
            ...
            <div className="space-y-2.5">
              ...
              <div className="rounded-xl border border-[#e5e5e5] bg-[#fafafa] p-3">
                <h4 className="text-xs font-black text-eel-dark-blue mb-1">
              ...
              {data.grammar?.clauses && data.grammar.clauses.length > 0 && (
                <div className="space-y-1.5">
                  ...
                  {data.grammar.clauses.map((clause, idx) => (
                    <div
                      key={idx}
                      className="rounded-xl border border-[#e5e5e5] bg-white p-2.5 space-y-1"
                    >
                      <div className="text-[10px] font-extrabold uppercase text-[#1cb0f6]">
                        {clause.role}
                      </div>
          ...
          {vocabCount === 0 && isEnriching ? (
            <div className="rounded-xl border border-[#e5e5e5] bg-[#fafafa] p-4 text-center space-y-1">
              ...
              <p className="text-xs font-black text-eel-dark-blue">
                Đang trích xuất từ vựng &amp; IPA theo ngữ cảnh...
              </p>
            </div>
          ) : vocabCount === 0 ? (
            <div className="py-4 text-center text-xs text-ash italic">
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        ...
                        <p className="mt-1 text-xs font-bold text-[#438f0e]">
