...
import type { POSTag, SentenceItem, ParagraphBlock } from "@/types/reading";
...
export function InteractiveReader({
  ...
  return (
    <div className="relative flex flex-col h-full">
      ...
      <div
        className={`flex-1 overflow-y-auto rounded-2xl border-2 border-[#e5e5e5] bg-white p-6 md:p-8 space-y-6 ${
          ...
          {sentence.tokens.map((token, tIdx) => {
            ...
            return (
              <span
                ...
                style={
                  shouldColor && highlightMode === "underline"
                    ? { borderBottomColor: style.textColor }
                    : undefined
                }
                className={`inline-block rounded-xs px-0.5 transition-colors cursor-help ${tokenClass} hover:bg-[#fff9d2] hover:text-black`}
              >
                {token.text}
              </span>
            );
