...
export async function POST(request: Request, { params }: { params: Promise<{ sessionId: string }> }) {
  ...
  const body = (await request.json().catch(() => null)) as {
    eventId?: unknown; wordId?: unknown; phase?: unknown; selectedWordId?: unknown; answer?: unknown;
  } | null;
  const phases: StudyPhase[] = ["flashcard", "multiple_choice", "typing"];
  if (!isUuid(sessionId) || !body || typeof body.eventId !== "string" || !isUuid(body.eventId) ||
    typeof body.wordId !== "string" || !isUuid(body.wordId) || typeof body.phase !== "string" ||
    !phases.includes(body.phase as StudyPhase) ||
    (body.phase === "multiple_choice" && (typeof body.selectedWordId !== "string" || !isUuid(body.selectedWordId))) ||
    (body.phase === "typing" && (typeof body.answer !== "string" || body.answer.length > 256))) {
  ...
  try {
    const submitted = await submitStudyEvent(db, {
      ...
      selectedWordId: typeof body.selectedWordId === "string" ? body.selectedWordId : undefined,
