...
import { getStudySession, StudyServiceError } from "@/lib/study-service";
...
export async function GET(request: Request, { params }: { params: Promise<{ sessionId: string }> }) {
  ...
  try {
    return NextResponse.json({ session: await getStudySession(db, sessionId, user.id) });
  } catch (error) {
    ...
    return NextResponse.json({ message: "Không thể tải phiên học." }, { status: 503 });
  }
}
